...
7. проверит отсутствие незакоммиченных изменений моделей, применит миграции, проверит суперпользователя и соберёт статику через entrypoint;
8. дождётся состояния `healthy`; при неработающем приложении деплой завершится ошибкой.

В production задаётся `MIGRATION_MODE=check`: entrypoint сначала выполняет `makemigrations --check --dry-run`, затем `migrate --noinput`. Сам Docker-образ при сборке дополнительно накатывает все миграции на временную SQLite-базу, поэтому повреждённая или неполная цепочка миграций не попадёт в деплой.
