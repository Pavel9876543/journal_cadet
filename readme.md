...
Для повседневной разработки используется быстрый набор без тяжёлого заполнения БД и конкурентного PostgreSQL-сценария:

```bash
./scripts/test-fast.sh
```

Тяжёлые проверки запускаются последовательно и не создают лишнюю нагрузку на процессор и базу данных:

```bash
./scripts/test-slow.sh
```

Полный набор одной командой:

```bash
./scripts/test-full.sh
```

По умолчанию быстрый набор использует не более двух процессов, полный и тяжёлый — один. Значения можно изменить через `TEST_PROCESSES`, а повторное использование тестовой БД отключить через `KEEP_TEST_DB=0`.
