...
Ошибки уровня `ERROR` и `CRITICAL` сохраняются в таблице «Журнал ошибок». В ней
фиксируются код ошибки, HTTP-статус, путь, метод, пользователь, источник и
трассировка. После появления 1001-й записи самые старые записи автоматически
удаляются: в базе всегда остаётся не более 1000 последних ошибок.
...
`ERROR_LOG_MAX_RECORDS` не может быть больше `1000`. Для development и test
хранение ошибок в БД по умолчанию отключено, но вывод в консоль сохраняется.
