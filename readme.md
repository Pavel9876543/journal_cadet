...
## Единая модель связей и доступа к журналу

Начиная с миграции `0038_canonical_journal_assignments` личный кабинет,
административные формы и API используют один слой `JournalAccessScope`.
Источники истины разделены однозначно:

- `StudentEnrollment` — участие ученика в конкретном учебном году и группе;
- `GroupSubject` — групповой предмет и назначенный преподаватель;
- `StudentSubject` — индивидуальный предмет и назначенный преподаватель;
- `AssessmentItem.responsible_teacher` — ответственный за произведение;
- `StudentAssessmentGroup` — назначение ученика в группу произведений.

Оценки и результаты (`Grade`, `SubjectResult`, `AssessmentResult`) являются
следствием назначений и сами по себе не открывают доступ. Таблицы
`TeacherEnrollment`, `TeacherSubject` и `UserAcademicYearMembership` остаются
служебными записями для истории и навигации; их отсутствие или устаревший флаг
не скрывает фактически назначенные преподавателю данные.
...
Для безопасной синхронизации только дублирующих полей и служебных записей:

```bash
python manage.py audit_journal_relations --fix
```

Команда не создаёт назначения из уже существующих оценок и результатов. Если
она сообщает о результате без явного назначения, связь нужно проверить и
создать администратору вручную.
